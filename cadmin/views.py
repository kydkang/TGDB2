...
def post_add(request):
    ...
    # if request is GET, show the unbound form to the user
    else:
        f = PostForm()
...
def post_list(request):
    if request.user.is_superuser:
        posts = Post.objects.order_by("-id").all() #may switch all() and order_by
    else:
    ...
    if request.method == "POST":
        ...
        if f.is_valid():
            if request.POST.get('author') == "" and request.user.is_superuser:
                ...
                new_category = f.save()
            else:                          # if author not a superuser
                new_category = f.save(commit=False)
...
def category_update(request, pk):
    ...
    # If request is POST, create a bound form(from existing category)
    if request.method == "POST":       
